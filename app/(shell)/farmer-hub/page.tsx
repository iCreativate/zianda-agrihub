...
function ExpandableCard({
  ...
  return (
    <>
      <article className="group overflow-hidden rounded-card border border-stone bg-paper shadow-soft transition hover:border-stone-strong hover:shadow-lift">
        <button
          type="button"
          onClick={() => onToggle(id)}
          aria-expanded={open}
          className="flex w-full items-center justify-between gap-4 p-5 text-left transition hover:bg-ivory/40"
        >
          <div className="flex min-w-0 items-start gap-3">
            <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-control bg-gradient-to-br from-crop-soft to-ivory-deep text-[11px] font-semibold uppercase tracking-wide text-crop ring-1 ring-crop/10">
              {iconLabel}
            </span>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {badge && (
                  <span className="rounded-full bg-crop-soft px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-crop">
                    {badge}
                  </span>
                )}
              </div>
              <p className="mt-1 text-sm leading-relaxed text-ink-muted">{summary}</p>
            </div>
          </div>
          <span className="inline-flex shrink-0 items-center gap-1.5 rounded-control border border-stone-strong bg-ivory-deep px-3 py-1.5 text-xs font-medium text-ink-muted transition group-hover:border-crop/25 group-hover:bg-crop-soft group-hover:text-crop">
            Open guide
        ...
        createPortal(
          ...
          >
            ...
            <div
              ...
              className="relative z-10 flex max-h-[92dvh] w-full max-w-4xl flex-col overflow-hidden rounded-t-card border border-stone bg-paper shadow-lift sm:max-h-[88dvh] sm:rounded-card"
            >
              <header className="shrink-0 border-b border-stone bg-gradient-to-br from-ivory via-paper to-crop-soft/35 px-5 py-5 md:px-7 md:py-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex min-w-0 items-start gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-control bg-paper text-xs font-semibold uppercase tracking-wide text-crop ring-1 ring-crop/15 shadow-soft">
                      {iconLabel}
                    </span>
                    <div className="min-w-0">
                      ...
                      <h2
                        id={`guide-title-${id}`}
                        className="mt-1 text-xl font-semibold tracking-tight text-ink md:text-2xl"
                      >
                        {title}
                      </h2>
                  ...
                  <button
                    type="button"
                    onClick={() => onToggle(id)}
                    className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-control border border-stone-strong bg-paper text-ink-muted transition hover:border-ink/15 hover:bg-ivory-deep hover:text-ink"
                    aria-label="Close"
                  >
                    <X className="h-4 w-4" />
                  </button>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 md:px-7 md:py-6">
                <div className="space-y-5 text-sm leading-relaxed text-ink-muted [&_h3]:text-base [&_h3]:font-semibold [&_h3]:text-crop [&_li]:leading-relaxed [&_table]:w-full [&_table]:text-xs [&_table]:text-ink [&_tbody_tr:nth-child(even)]:bg-ivory/50 [&_td]:border-b [&_td]:border-stone [&_td]:px-3 [&_td]:py-2.5 [&_th]:border-b [&_th]:border-stone [&_th]:px-3 [&_th]:py-2.5 [&_th]:text-left [&_th]:text-[11px] [&_th]:font-semibold [&_th]:uppercase [&_th]:tracking-wide [&_th]:text-ink-muted [&_thead]:bg-ivory-deep [&_ul]:space-y-1.5">
                  {children}
                </div>
              </div>

              <footer className="flex shrink-0 items-center justify-between gap-3 border-t border-stone bg-ivory/70 px-5 py-4 md:px-7">
                <p className="text-xs text-ink-subtle">
                  Values are examples — adapt with your supplier or animal health technician.
                </p>
                <button type="button" onClick={() => onToggle(id)} className="btn-secondary min-h-10 shrink-0">
                  Close
                </button>
              </footer>
            </div>
...
export default function FarmerHubPage() {
  ...
  const toggle = useCallback(
    (id: SectionId) => setOpenSections((prev) => ({ ...prev, [id]: !prev[id] })),
    []
  );
  ...
  return (
    <AppPage
      hero={
        <PageHero
        eyebrow="Crops & Livestock"
        title="Farmer Hub: Feeding Guides"
        description="Practical, field-ready guidelines to help you plan feed for different animal groups and crop uses on your farm."
        image="/images/home/livestock.jpg"
        imageAlt="Livestock feeding on the farm"
        asideTitle="Field notes"
        asideNote="Values are examples. Check feed labels and adapt with your supplier or animal health technician."
        />
      }
    >

      <section className="space-y-4 card-shell">
        <ExpandableCard
          id="broilers"
        ...
        >
          <div className="overflow-x-auto rounded-xl border border-stone bg-ivory-deep text-xs text-ink">
            <table className="min-w-full border-collapse">
              <thead className="bg-paper text-[11px] uppercase tracking-wide text-ink-muted">
                <tr>
                  <th className="border-b border-stone px-3 py-2 text-left">
                    Description (g/kg)
                  </th>
                  <th className="border-b border-stone px-3 py-2 text-left">
                    Starter BA220
              ...
              <tbody>
                <tr className="bg-transparent">
                  <td className="border-b border-stone px-3 py-2">
                    Protein (min)
                  </td>
                  <td className="border-b border-stone px-3 py-2">220</td>
                  <td className="border-b border-stone px-3 py-2">180</td>
                  <td className="border-b border-stone px-3 py-2">160</td>
                  <td className="border-b border-stone px-3 py-2">140</td>
                </tr>
                <tr>
                  <td className="border-b border-stone px-3 py-2">
                    Methionine (min)
                  </td>
                  <td className="border-b border-stone px-3 py-2">4.4</td>
                  <td className="border-b border-stone px-3 py-2">4.0</td>
                  <td className="border-b border-stone px-3 py-2">3.6</td>
                  <td className="border-b border-stone px-3 py-2">-</td>
                </tr>
                <tr className="bg-transparent">
                  <td className="border-b border-stone px-3 py-2">
                    Lysine (min)
                  </td>
                  <td className="border-b border-stone px-3 py-2">11</td>
                  <td className="border-b border-stone px-3 py-2">9.5</td>
                  <td className="border-b border-stone px-3 py-2">9</td>
                  <td className="border-b border-stone px-3 py-2">6</td>
                </tr>
                <tr>
                  <td className="border-b border-stone px-3 py-2">
                    Moisture (max)
                  </td>
                  <td className="border-b border-stone px-3 py-2">120</td>
                  <td className="border-b border-stone px-3 py-2">120</td>
                  <td className="border-b border-stone px-3 py-2">120</td>
                  <td className="border-b border-stone px-3 py-2">120</td>
                </tr>
                <tr className="bg-transparent">
                  ...
                  <td className="border-b border-stone px-3 py-2">25</td>
                  <td className="border-b border-stone px-3 py-2">25</td>
                  <td className="border-b border-stone px-3 py-2">25</td>
                  <td className="border-b border-stone px-3 py-2">25</td>
                </tr>
                <tr>
                  ...
                  <td className="border-b border-stone px-3 py-2">50</td>
                  <td className="border-b border-stone px-3 py-2">50</td>
                  <td className="border-b border-stone px-3 py-2">70</td>
                  <td className="border-b border-stone px-3 py-2">70</td>
                </tr>
                <tr className="bg-transparent">
                  <td className="border-b border-stone px-3 py-2">
                  ...
                  <td className="border-b border-stone px-3 py-2">8</td>
                  <td className="border-b border-stone px-3 py-2">7</td>
                  <td className="border-b border-stone px-3 py-2">6</td>
                  <td className="border-b border-stone px-3 py-2">7</td>
                </tr>
                <tr>
                  ...
                  <td className="border-b border-stone px-3 py-2">12</td>
                  <td className="border-b border-stone px-3 py-2">12</td>
                  <td className="border-b border-stone px-3 py-2">12</td>
                  <td className="border-b border-stone px-3 py-2">12</td>
                </tr>
                <tr className="bg-transparent">
                  ...
                  <td className="border-b border-stone px-3 py-2">7</td>
                  <td className="border-b border-stone px-3 py-2">6</td>
                  <td className="border-b border-stone px-3 py-2">5</td>
                  <td className="border-b border-stone px-3 py-2">5</td>
                </tr>
                <tr>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="space-y-2 rounded-xl border border-stone/70 bg-ivory-deep p-4 text-xs text-ink">
              <h3 className="text-sm font-semibold text-crop">
          ...
          <div className="mt-4 space-y-1.5 text-xs text-ink-muted">
            <p className="font-semibold text-crop">
      ...
      <section className="grid gap-6 md:grid-cols-2">
        ...
        >
          <table className="min-w-full border-collapse text-xs">
            ...
            <tbody>
              <tr className="bg-transparent">
                <td className="border-b border-stone px-3 py-2">A</td>
                <td className="border-b border-stone px-3 py-2">
              ...
              <tr>
                <td className="border-b border-stone px-3 py-2">B</td>
                <td className="border-b border-stone px-3 py-2">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-stone bg-paper px-4 py-3 text-xs text-ink-muted">
        <p>
        ...
        <Link
          ...
          className="inline-flex items-center gap-1 rounded-lg border border-stone bg-crop-soft px-3 py-1.5 text-xs font-medium text-crop hover:bg-crop-soft"
        >
